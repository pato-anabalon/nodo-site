...
describe('/api/contact', () => {
  ...
  it('should reject honeypot and too-fast submissions', async () => {
    const response = await POST(
      new Request('https://nodo.test/api/contact', {
        method: 'POST',
  ...
  it('should accept a valid lead without attachments', async () => {
    ...
    expect(contactIntegrations.forwardContactLead).toHaveBeenCalledWith(
  ...
  it('should validate attachments', async () => {
    ...
    expect(response.status).toBe(200);
    expect(contactServer.storeContactAttachment).toHaveBeenCalledWith(
      expect.objectContaining({
