...
describe('/api/contact/upload', () => {
  ...
  it('should return handleUploadPresigned JSON', async () => {
    ...
    const response = await POST(
      new Request('https://nodo.test/api/contact/upload', {
        method: 'POST',
  ...
  it('should issue signed tokens for valid upload metadata', async () => {
    (handleUploadPresigned as jest.Mock).mockImplementation(async ({ getSignedToken }) =>
      getSignedToken(`contact/2026-06-15/${validSubmissionId}/file-1.pdf`, JSON.stringify(validPayload))
    );
