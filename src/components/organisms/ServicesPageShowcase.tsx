      ...
      {services.map((service, index) => {
        ...
        return (
          <ScrollReveal key={service.title} delay={index * 0.04}>
            <article
              data-testid={`services-showcase-card-${slug}`}
              className="group relative overflow-hidden rounded-[2rem] border border-white/12 bg-white/[0.04] p-6 transition duration-300 hover:border-nodo-purple/55 hover:bg-white/[0.06] hover:shadow-[0_18px_54px_rgba(124,58,237,0.18)] sm:p-8 lg:p-10"
            >
              <div
                className={cn(
              ...
              <div className="relative grid gap-8 lg:grid-cols-[minmax(0,1.1fr)_minmax(18rem,0.9fr)] lg:gap-10">
                <div
                  data-testid={`services-showcase-card-${slug}-content`}
                  className={cn('relative space-y-6 overflow-hidden', isReversed && 'lg:order-2')}
                >
                  <span
                    aria-hidden="true"
                  ...
                  <div className="relative z-10 flex items-start justify-between gap-4">
                    <div>
                      <p
                        data-testid={`services-showcase-card-${slug}-eyebrow`}
                        className="text-xs font-semibold uppercase tracking-[0.26em] text-nodo-lavender/90"
                      >
                        {service.eyebrow}
                      </p>
                      <h3
                        data-testid={`services-showcase-card-${slug}-title`}
                        className="mt-4 max-w-xl text-balance text-3xl font-black leading-[0.92] tracking-[-0.03em] text-white sm:text-4xl lg:text-[3.25rem]"
                      >
                        {service.title}
                      </h3>
                    </div>
                    <div
                      data-testid={`services-showcase-card-${slug}-icon`}
                      className="flex size-14 shrink-0 items-center justify-center rounded-2xl border border-white/12 bg-white/8 text-white shadow-[0_0_36px_rgba(124,58,237,0.18)] sm:size-16"
                    >
                      <Icon aria-hidden="true" className="size-6" />
                    </div>
                  </div>
                  <p
                    data-testid={`services-showcase-card-${slug}-description`}
                    className="relative z-10 max-w-2xl text-pretty text-base leading-8 text-white/68 sm:text-lg"
                  >
                    {service.description}
                  </p>
                  <div
                  ...
                  >
                    <p className="text-xs font-black uppercase tracking-[0.18em] text-nodo-lavender">Best when</p>
                    <p className="mt-2 text-sm font-semibold leading-6 text-white/72">{service.bestWhen}</p>
                  </div>
                  <div
                    data-testid={`services-showcase-card-${slug}-highlight`}
                    className="relative z-10 flex flex-wrap items-center gap-4 border-t border-white/10 pt-6"
                  >
                ...
                >
                  <p className="text-sm font-semibold uppercase tracking-[0.2em] text-white/42">What this includes</p>
                  <ul className="mt-5 space-y-4">
                    {service.deliverables.map((item) => (
                      <li key={item} className="flex gap-3 text-sm leading-7 text-white/74 sm:text-base">
